...
export default function HomePage() {
  ...
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<string>("") // "" means all categories
  ...
  const tableHeaders = useMemo(() => {
    ...
    const specialHeaders = ["Computed Latitude", "Computed Longitude", "Parsed Coordinate Segment"]
  ...
  useEffect(() => {
    if (selectedPointId && tableBodyRef.current) {
      const rowElement = tableBodyRef.current.querySelector(`[data-point-id="${selectedPointId}"]`) as HTMLElement
  ...
  const filteredPoints = useMemo(() => {
    ...
    if (selectedCategoryFilter && selectedCategoryFilter !== "all") {
      // Check against "all"
      currentPoints = currentPoints.filter((point) => point.category === selectedCategoryFilter)
    }

    if (searchQuery.trim() !== "") {
      const lowerCaseQuery = searchQuery.toLowerCase()
      currentPoints = currentPoints.filter((point) => {
        if (point.originalName?.toLowerCase().includes(lowerCaseQuery)) return true
        if (point.category?.toLowerCase().includes(lowerCaseQuery)) return true
        if (point.originalCoords?.toLowerCase().includes(lowerCaseQuery)) return true
        return Object.values(point.rowData).some((value) => String(value).toLowerCase().includes(lowerCaseQuery))
      })
    }
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-gray-100">
      ...
      <main className="flex-grow flex flex-row overflow-hidden">
        ...
        <div className="w-2/5 p-4 flex flex-col space-y-4 border-r border-gray-300 bg-white overflow-hidden">
          <div className="flex-shrink-0 space-y-3 mb-4 p-3 bg-slate-50 rounded-md border border-slate-200">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label htmlFor="search-table" className="block text-sm font-medium text-slate-600 mb-1">
                  Search Table
                </label>
                <Input
                  id="search-table"
              ...
              <div>
                <label htmlFor="category-filter" className="block text-sm font-medium text-slate-600 mb-1">
                  Filter by Category
                </label>
                <Select
          ...
          <div className="flex-shrink-0">
            <h2 className="text-lg font-semibold mb-2 text-slate-700">Upload CSV File</h2>
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <Input type="file" accept=".csv" onChange={handleFileChange} className="max-w-xs" disabled={isPending} />
              {fileName && !isPending && <span className="text-sm text-gray-600">Selected: {fileName}</span>}
            </div>
            {isPending && (
          ...
          <div className="flex-grow overflow-hidden flex flex-col">
            {!error && (points.length > 0 || searchQuery || selectedCategoryFilter) && !isPending && (
              <>
                <h3 className="text-md font-semibold text-slate-700 mb-2 flex-shrink-0">
                  Displaying {filteredPoints.length} (of {points.length} total) from {fileName}
                </h3>
                    ...
                    <tbody ref={tableBodyRef} className="bg-white divide-y divide-gray-200">
                      {filteredPoints.map((point) => (
                        <tr
                          ...
                          className={`cursor-pointer hover:bg-slate-50 ${selectedPointId === point.id ? "bg-slate-200 ring-2 ring-slate-400" : ""}`}
                        >
                          {tableHeaders.map((header) => {
            ...
            {!error && points.length === 0 && !isPending && fileName && (
              <div className="p-3 bg-yellow-100 text-yellow-700 rounded flex-grow flex items-center justify-center text-sm">
                <AlertCircle className="h-5 w-5 mr-2 flex-shrink-0" />
                No points extracted from {fileName}. Check file format and column names.
              </div>
            )}
            ...
            {isPending && points.length === 0 && (
              <div className="p-3 text-slate-600 rounded flex-grow flex items-center justify-center text-sm">
                <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading data for table...
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-grow h-full bg-gray-200">
          <MapDisplay points={filteredPoints} selectedPointId={selectedPointId} onPointSelect={handlePointSelect} />
        </div>
      </main>
    </div>
  )
